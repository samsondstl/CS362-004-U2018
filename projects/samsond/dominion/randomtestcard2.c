...
int main()
{
   ...
   for (run = 0; run < numRuns; run++)
   {
      ...
      gainCard(village, g, 2, currPlayer);

      prevState = state;

      printf("VILLAGE: Run = %d Player = %d\n", run + 1, currPlayer + 1);
      cardEffect(village, rand() % 10, rand() % 10, rand() % 10, g, state.handCount[currPlayer] - 1, 0);      
      ...
      for (player = 0; player < numPlayers; player++)
      {
         if (player == currPlayer)
         {
            ...
            assertTrue(state.numActions           == prevState.numActions + 2);
         }
         else
         {
            assertTrue(state.handCount[player]    == prevState.handCount[player]);
